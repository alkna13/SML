...
doKnn <- function(xPca, saveResults) {
  ...
  while (k <= k_end)
  {
    ...
    data_out[row, 14] <<- minVote
    data_out[row, 15] <<- doubts
...
personsNrTotal=7 #number of persons in selected groups
...
x = loadMultiplePersonsData(dpi, groupStart, groupEnd, pathToGroups, smoothing, sigma)#mult. person
...
data_out <- array(0, dim = c(k_runs, 15))
colnames(data_out) <-
  c(
    "Group_#",
    "Member#",
